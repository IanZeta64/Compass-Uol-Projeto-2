...
public class AdoptionDocControllerImpl implements AdoptionDocController {
    ...
    public ResponseEntity<Void> delete(String id) {
        try {
            service.delete(id);
            return ResponseEntity.noContent().build(); // Resposta 204 (NO_CONTENT) para indicar sucesso na exclusão
        } catch (AdoptionDocNotFoundException e) {
            return ResponseEntity.notFound().build(); // Resposta 404 (NOT_FOUND) quando o ID não for encontrado
        }
    }
}
